...
- **RTO program details** — Full flow from **Jelajahi program lain** → dealer → motor:
  - Dealer list with nearest-branch distance and sort-by-location when GPS is available.
  - **Dealer page**: program banner, WhatsApp share, branch pills + km, Google Maps, hours, call/WhatsApp, dealership photos, motor list with Harian / Min. gaji.
  - **Motor page**: angle gallery, daily rate × program days & min salary, specs & charging (nearby stations), payment schedule + estimated completion date, benefits, apply + WhatsApp share.
  - Payment completion estimate excludes weekends; based on “today” for a rolling date. Mock catalog supports multiple bikes per operator.
